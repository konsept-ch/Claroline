...
!function () {
  'use strict'
  window.actions = [{id: 'alef', mml: '<mo>&#8501;</mo>', latex: '\\aleph'}, {
  ...
  'use strict'
  window.panels = [{
    id: 'symbols', width: 334, visible: !0, sections: [{
      id: 'basic-math',
    ...
    }, {id: 'spaces', children: ['normal-space', 'digit-space', 'thinner-space', 'back-space']}]
  }, {
    id: 'integrals-limit',
    width: 388,
    visible: !1,
    ...
    }, {id: 'union', children: ['big-union', 'big-intersection', 'big-square-cup', 'big-square-cap']}]
  }, {
    id: 'matrices',
    width: 175,
    visible: !1,
    ...
    }, {id: 'equation-parts', children: ['aligned-equations', 'piecewise-function']}]
  }, {
    id: 'decorations',
    width: 205,
    visible: !1,
  ...
  'use strict'
  window.AreaPanel = function (a, b) {
    this.visible = a.visible, this.id = a.id, this.sectionWidth = a.width, this.element = document.createElement('div'), this.element.className = 'fm-editor-panel-area', this.element.id = this.id + '-panel-area', this.toolbar = b, this.sections = [], this.panel = a, this.createPanelArea(), this.createSections(a.sections)
    ...
    createPanelArea: function () {
      ...
      var b = this.panelButtonArea.getElementsByClassName('fm-editor-panel-area-popup-button')[0]
      b.addEventListener('click', function (b) {
        a.toggleSections(b)
      })
      var c = this.panelButtonArea.getElementsByClassName('fm-editor-panel-area-movedown-button')[0]
      c.addEventListener('click', function (b) {
        a.scrollDownAreaBox()
      })
      var d = this.panelButtonArea.getElementsByClassName('fm-editor-panel-area-moveup-button')[0]
      d.addEventListener('click', function (b) {
        a.scrollUpAreaBox()
      })
    },
    createSections: function (a) {
      var b = document.createElement('div')
      b.className = 'fm-editor-panel-area-section-container'
      var c = document.createElement('div')
      c.className = 'fm-editor-panel-area-section-box', c.style.width = this.sectionWidth + 'px', b.appendChild(c)
      var d = document.createElement('div')
      d.className = 'fm-editor-panel-area-section-box-container', c.appendChild(d)
      var e = document.createElement('div')
      e.className = 'fm-editor-panel-area-section-overlap-container', d.appendChild(e)
      var f = new DropdownList(this)
      e.appendChild(f.element)
      for (var g = 0; g < a.length; g++) {
        var h = a[g]
        this.appendSection(h, e, f, 0 == g)
      }
      this.sectionsMountPoint = document.createElement('div'), this.sectionsMountPoint.className = 'fm-editor-panel-area-section-mount-point', this.sectionsMountPoint.appendChild(b), this.element.appendChild(this.sectionsMountPoint)
    },
    toggleSections: function (a) {
      return null != this.sectionsMountPoint && (this.active ? this.hideSections(a) : this.showSections(a)), a.stopPropagation(), !1
    },
    showSections: function (a) {
      null != this.toolbar.activePanel && this.toolbar.activePanel.hideSections(a), this.sectionsMountPoint.style.display = 'block', this.active = !0, DomUtils.addClass(this.element, 'active'), this.toolbar.activePanel = this
    },
    hideSections: function (a) {
    ...
    appendSection: function (a, b, c, d) {
      var e = new ButtonSection(a, this)
      e.buttons.length > 0 && (b.appendChild(e.element), this.sections.push(e), c.addListItem(e, d))
    },
    cloneButtonsToAreaBox: function (a) {
      this.panelAreaBox.innerHTML = ''
      var b = document.createElement('div')
      b.className = 'fm-editor-section-buttons', this.panelAreaBox.appendChild(b)
      for (var c = 0; c < a.buttons.length; c++) {
        var d = a.buttons[c], e = new Button(d.id, a)
        b.appendChild(e.element)
      }
      this.panelAreaBox.style.marginTop = '0px'
    },
    scrollUpAreaBox: function () {
      var a = this.getMarginTop()
      this.panelAreaBox.style.marginTop = Math.min(0, a + 56) + 'px'
    },
    scrollDownAreaBox: function () {
      var a = parseInt(this.panelAreaBox.scrollHeight), b = this.getMarginTop()
      this.panelAreaBox.style.marginTop = Math.min(0, Math.max(-a + 56 + a % 28, b - 56)) + 'px'
    },
    getMarginTop: function () {
      var a = this.panelAreaBox.style.marginTop
      return 0 == a.length && (a = 0), parseInt(a)
    },
    redraw: function () {
  ...
  'use strict'
  window.Button = function (a, b) {
    this.id = a, this.element = document.createElement('span'), this.element.className = 'fm-editor-button math math-' + a, this.element.id = this.id + '-btn', this.section = b, this.createEvent()
    var a = this
    this.element.addEventListener('click', function (b) {
      a.addEquationEvent.clientX = b.clientX, a.addEquationEvent.clientY = b.clientY, a.element.dispatchEvent(a.addEquationEvent)
    })
    ...
    }, element: null, addEquationEvent: null, id: null, section: null, __class__: Button
  }
}(), function () {
  'use strict'
  window.ButtonPanel = function (a, b) {
    this.visible = a.visible, this.id = a.id, this.name = trans[a.id] || a.id, this.sectionWidth = a.width, this.element = document.createElement('div'), this.element.className = 'fm-editor-panel', this.element.id = this.id + '-panel', this.toolbar = b, this.panel = a, this.sections = [], this.createPanelButton(), this.createSections(a.sections)
    ...
    createPanelButton: function () {
      ...
      a.className = 'fm-editor-panel-button-icon math math-' + this.id + '-btn'
      var b = document.createElement('div')
      b.className = 'fm-editor-panel-button-label', b.innerHTML = this.name
      var c = document.createElement('div')
      c.className = 'arrow-down', this.buttonElement.appendChild(a), this.buttonElement.appendChild(b), this.buttonElement.appendChild(c), this.element.appendChild(this.buttonElement)
      var d = this
      this.buttonElement.addEventListener('click', function (a) {
        d.toggleSections(a)
      })
    },
    createSections: function (a) {
      var b = document.createElement('div')
      b.className = 'fm-editor-section-container', b.style.width = this.sectionWidth + 'px'
      for (var c = 0; c < a.length; c++) {
        var d = a[c]
        this.appendSection(d, b)
      }
      this.sectionsMountPoint = document.createElement('div'), this.sectionsMountPoint.className = 'fm-editor-section-mount-point', this.sectionsMountPoint.appendChild(b), this.element.appendChild(this.sectionsMountPoint)
    },
    toggleSections: function (a) {
      return null != this.sectionsMountPoint && (this.active ? this.hideSections(a) : this.showSections(a)), this.toolbar.mlangPanel.hideList(), a.stopPropagation(), !1
    },
    showSections: function (a) {
      null != this.toolbar.activePanel && this.toolbar.activePanel.hideSections(a), this.active = !0, DomUtils.addClass(this.element, 'active')
      var b = this.buttonElement.getBoundingClientRect().left + this.sectionWidth
      b > document.body.clientWidth ? this.sectionsMountPoint.style.left = -this.sectionWidth + this.buttonElement.clientWidth + 'px' : this.sectionsMountPoint.style.left = '-1px', this.toolbar.activePanel = this
    },
    hideSections: function (a) {
      this.active = !1, DomUtils.removeClass(this.element, 'active'), this.toolbar.activePanel = null
    },
    appendSection: function (a, b) {
      var c = new ButtonSection(a, this)
      c.buttons.length > 0 && (b.appendChild(c.element), this.sections.push(c))
    },
    redraw: function () {
  ...
  'use strict'
  window.ButtonSection = function (a, b) {
    this.id = a.id, this.name = a.name || trans[a.id] || a.id, this.element = document.createElement('div'), this.element.className = 'fm-editor-section', this.element.id = this.id + '-section', this.panel = b, this.createTitle(), this.buttons = [], a.children && a.children.length > 0 && this.createButtons(a.children)
  }, ButtonSection.__name__ = ['ButtonSection'], ButtonSection.prototype = {
    createButtons: function (a) {
      var b = this.panel.toolbar.editor.actions, c = this.panel.toolbar.editor.mlang, d = document.createElement('div')
      d.className = 'fm-editor-section-buttons'
      for (var e = 0; e < a.length; e++) {
        var f = a[e], g = b.get(f)
        if (g && g[c]) {
          var h = new Button(f, this)
          d.appendChild(h.element), this.buttons.push(h)
        }
      }
      this.element.appendChild(d)
    }, createTitle: function () {
      var a = document.createElement('div')
      a.className = 'fm-editor-section-title', a.innerHTML = this.name, this.element.appendChild(a)
    }, setActive: function () {
    ...
    }, element: null, active: !1, buttons: null, id: null, name: null, panel: null, __class__: ButtonSection
  }
}(), function () {
  'use strict'
  window.DropdownList = function (a) {
    this.areaPanel = a, this.element = document.createElement('div'), this.element.className = 'fm-editor-dropdown fm-editor-dropdown-button', this.element.id = a.id + '-dropdown', this.createLabel(), this.createList()
    ...
    createLabel: function () {
      ...
      var a = document.createElement('div')
      a.className = 'arrow-down', this.labelButton.appendChild(a), this.element.appendChild(this.labelButton)
      var b = this
      this.labelButton.addEventListener('click', function (a) {
        b.toggleList(a)
      })
    },
    createList: function () {
      ...
      var a = document.createElement('div')
      a.className = 'fm-editor-dropdown-list-items', this.listItemContainer = document.createElement('div'), this.listItemContainer.className = 'fm-editor-dropdown-list-items-container', a.appendChild(this.listItemContainer), this.listMountPoint.appendChild(a), this.element.appendChild(this.listMountPoint)
    },
    toggleList: function (a) {
      return this.active ? this.hideList(a) : this.showList(a), a.stopPropagation(), !1
    },
    hideList: function (a) {
    ...
    addListItem: function (a, b) {
      var c = new DropdownListItem(a, this)
      this.listItemContainer.appendChild(c.element), b && c.setActive()
    },
    changeActiveItem: function (a) {
      null !== this.activeItem && this.activeItem.unsetActive(), this.activeItem = a, this.labelButtonText.innerHTML = a.name, this.areaPanel.cloneButtonsToAreaBox(this.activeItem.section)
    },
    element: null,
    active: !1,
    labelButton: null,
  ...
  'use strict'
  window.DropdownListItem = function (a, b) {
    this.id = a.id, this.name = a.name, this.element = document.createElement('div'), this.element.className = 'fm-editor-dropdown-list-item', this.element.id = this.id + '-list-item', this.section = a, this.dropdownList = b, this.element.innerHTML = this.name, this.addListener()
    ...
    addListener: function () {
      ...
      return this.setActive(), this.dropdownList.hideList(a), a.stopPropagation(), !1
    }, setActive: function () {
    ...
    }, element: null, active: !1, id: null, name: null, section: null, dropdownList: null, __class__: DropdownListItem
  }
}(), function () {
  'use strict'
  window.Editor = function (a, b, c) {
    this.lang = c.lang || this.lang, this.lang = this.lang.replace(/_[a-zA-Z]+/g, '').toLocaleLowerCase(), this.mlang = c.mlang || this.mlang, this.initEquation = c.equation || this.initEquation, this.buildActionsHash(a), this.panels = b
    var d = this
    ...
    init: function () {
      ...
      var a = this
      this.element.addEventListener('addEquation', function (b) {
        a.addEquation(b)
      }), document.addEventListener('click', function (b) {
        null != a.toolbar.activePanel && a.toolbar.activePanel.hideSections(b), a.toolbar.mlangPanel.hideList(), (-1 == b.target.id.indexOf('btn') || -1 == b.target.id.indexOf('table') && -1 == b.target.id.indexOf('matrix')) && a.hideMatrixPopup()
      }), null !== this.initEquation && this.insertEquationToTextarea(decodeURIComponent(this.initEquation))
    },
    createToolbar: function (a) {
      this.toolbar = new Toolbar(a, this), this.element.appendChild(this.toolbar.element)
    },
    addEquation: function (a) {
      var b = this.actions.get(a.formulaAction), c = b[this.mlang]
      if (b.matrix)this.currentMatrixEquation = c, this.showMatrixPopup(a.clientX, a.clientY); else {
        var d = /\{\{\$\}\}/g, e = c.match(d)
        if (e && e.length > 0)if (1 == e)c = c.replace(d, 'x'); else {
          var f = 1
          c = c.replace(d, function () {
            return 'x' + f++
          })
        }
        c = c.trim(), this.insertEquationToTextarea(c)
      }
    },
    insertEquationToTextarea: function (a) {
      'latex' == this.mlang && (a += ' '), this.textarea.insertAtCaret(a), this.renderEquationToResultarea(this.textarea.value)
    },
    insertMatrixToTextarea: function () {
      var a = parseInt(document.getElementById('fm-editor-matrix-rows').value), b = parseInt(document.getElementById('fm-editor-matrix-columns').value), c = this.currentMatrixEquation
      if (this.hideMatrixPopup(), null !== c && a > 0 && b > 0) {
        var d = ''
        d = 'latex' == this.mlang ? this.createMatrixCodeLatex(a, b) : this.createMatrixCodeMml(a, b), c = c.replace(/\{\{\$\}\}/g, d), c = c.trim(), this.insertEquationToTextarea(c)
      }
    },
    createMatrixCodeLatex: function (a, b) {
      for (var c = '', d = 0; a > d; d++) {
        for (var e = 0; b > e; e++)c += ' x_' + (e + 1 + d * b), c += b - 1 > e ? ' &' : ' '
        a - 1 > d && (c += '\\\\')
      }
      return c
    },
    createMatrixCodeMml: function (a, b) {
      for (var c = '', d = 0; a > d; d++) {
        c += '<mtr>'
        for (var e = 0; b > e; e++)c += '<mtd><msub><mi>x</mi><mi>' + (e + 1 + d * b) + '</mi></msub></mtd>'
        c += '</mtr>'
      }
      return c
    },
    renderEquationToResultarea: function (a) {
      a = a || '', '' !== a.trim() ? (a = 'latex' == this.mlang ? '$$' + a.trim() + '$$' : '<math xmlns="http://www.w3.org/1998/Math/MathML" mode="display">' + a.trim() + '</math>', this.toolbar.mlangPanel.disable(), this.resultarea.innerHTML = a, MathJax.Hub.Queue(['Typeset', MathJax.Hub, this.resultarea])) : (this.toolbar.mlangPanel.enable(), this.resultarea.innerHTML = '')
    },
    createTextArea: function () {
      var a = document.createElement('div')
      a.className = 'fm-editor-content-area', this.textarea = document.createElement('textarea'), this.textarea.className = 'fm-editor-content', a.appendChild(this.textarea), this.element.appendChild(a)
      var b = this
      this.textarea.addEventListener('input', function () {
        b.renderEquationToResultarea(this.value)
      })
    },
    createResultArea: function () {
      var a = document.createElement('div')
      a.className = 'fm-editor-result-area'
      var b = document.createElement('div')
      b.className = 'fm-editor-result-area-label', b.innerHTML = trans.result || 'result', this.resultarea = document.createElement('div'), this.resultarea.className = 'fm-editor-result-area-inner', a.appendChild(b), a.appendChild(this.resultarea), this.element.appendChild(a)
    },
    createMatixActionPopup: function () {
      ...
      var a = document.createElement('div')
      a.className = 'fm-editor-matrix-popup-container', a.innerHTML = '<table><tr><td>' + (trans.rows || 'rows') + ':</td><td><input id="fm-editor-matrix-rows" type=\'number\' name=\'matrix-rows\'/></td></tr><tr><td>' + (trans.columns || 'columns') + ':</td><td><input id="fm-editor-matrix-columns" type=\'number\' name=\'matrix-columns\'/></td></tr><tr><td colspan=\'2\'><button type=\'button\' id=\'fm-editor-matrix-create-btn\'>' + (trans.ok || 'ok') + '</button></td></tr></table>', this.matrixPopupMount.appendChild(a), this.element.appendChild(this.matrixPopupMount)
      var b = this
      this.matrixPopupMount.addEventListener('click', function (a) {
        return a.stopPropagation(), !1
      }), document.getElementById('fm-editor-matrix-create-btn').addEventListener('click', function () {
        b.insertMatrixToTextarea()
      }), document.getElementById('fm-editor-matrix-rows').addEventListener('keypress', function (a) {
        (13 == a.which || 13 == a.keyCode) && document.getElementById('fm-editor-matrix-columns').focus()
      }), document.getElementById('fm-editor-matrix-columns').addEventListener('keypress', function (a) {
        (13 == a.which || 13 == a.keyCode) && b.insertMatrixToTextarea()
      })
    },
    showMatrixPopup: function (a, b) {
      var c = document.getElementById('fm-editor-matrix-rows')
      c.value = 2, document.getElementById('fm-editor-matrix-columns').value = 2, DomUtils.addClass(this.matrixPopupMount, 'active'), this.matrixPopupMount.style.top = b - 40 + 'px', this.matrixPopupMount.style.left = a - 20 + 'px', c.focus()
    },
    hideMatrixPopup: function () {
      DomUtils.removeClass(this.matrixPopupMount, 'active'), this.currentMatrixEquation = null
    },
    buildActionsHash: function (a) {
      this.actions = new HashArray
      for (var b = 0; b < a.length; b++) {
        var c = a[b]
        this.actions.set(c.id, c)
      }
    },
    getEquationPng: function (a) {
      var b = this.resultarea.getElementsByTagName('svg')[0]
      if (b) {
        b = b.cloneNode(!0)
        var c = this
        DomUtils.replaceSVGUseWithGraphElements(b)
        var d = new Image
        d.onload = function () {
          ...
          e.drawImage(d, 0, 0)
          var f = b.toDataURL('image/png'), g = c.mlang, h = c.textarea.value.trim()
          a(f, g, h)
        }
        var e = (new XMLSerializer).serializeToString(b)
        d.src = 'data:image/svg+xml,' + encodeURIComponent(e)
  ...
  'use strict'
  window.MlangPanel = function (a) {
    this.id = 'mlang', this.element = document.createElement('div'), this.element.className = 'fm-editor-panel', this.toolbar = a, this.currentMlang = this.toolbar.editor.mlang, this.createPanelButton(), this.createList()
  }, MlangPanel.__name__ = ['MlangPanel'], MlangPanel.prototype = {
    createPanelButton: function () {
      var a = document.createElement('div')
      a.className = 'fm-editor-panel-button'
      var b = document.createElement('div')
      b.className = 'fm-editor-panel-button-icon math math-' + this.id + '-btn', this.labelButtonText = document.createElement('div'), this.labelButtonText.className = 'fm-editor-panel-button-label', this.labelButtonText.innerHTML = this.mlangs[this.currentMlang]
      var c = document.createElement('div')
      c.className = 'arrow-down', a.appendChild(b), a.appendChild(this.labelButtonText), a.appendChild(c), this.element.appendChild(a)
      var d = this
      a.addEventListener('click', function (a) {
        return d.enabled && d.toggleList(a), null != d.toolbar.activePanel && d.toolbar.activePanel.hideSections(a), a.stopPropagation(), !1
      })
    },
    createList: function () {
      this.listMountPoint = document.createElement('div'), this.listMountPoint.className = 'fm-editor-dropdown-list-mount-point'
      var a = document.createElement('div')
      a.className = 'fm-editor-dropdown-list-items', this.listItemContainer = document.createElement('div'), this.listItemContainer.className = 'fm-editor-dropdown-list-items-container'
      var b = new ButtonSection({id: 'latex', name: 'LaTeX', children: []}, this), c = new DropdownListItem(b, this)
      this.listItemContainer.appendChild(c.element)
      var d = new ButtonSection({id: 'mml', name: 'MathML', children: []}, this), e = new DropdownListItem(d, this)
      this.listItemContainer.appendChild(e.element), 'latex' == this.currentMlang ? c.setActive() : e.setActive(), a.appendChild(this.listItemContainer), this.listMountPoint.appendChild(a), this.element.appendChild(this.listMountPoint)
    },
    toggleList: function (a) {
    ...
    changeActiveItem: function (a) {
      null != this.activeItem && (this.activeItem.unsetActive(), this.toolbar.editor.mlang = a.id, this.toolbar.redraw()), this.activeItem = a, this.labelButtonText.innerHTML = a.name
    },
    disable: function () {
  ...
  'use strict'
  window.Toolbar = function (a, b) {
    this.element = document.createElement('div'), this.element.className = 'fm-editor-toolbar', this.editor = b, this.createPanels(a)
  }, Toolbar.__name__ = ['Toolbar'], Toolbar.prototype = {
    createPanels: function (a) {
      this.panels = [], this.activePanel = null
      var b = document.createElement('div')
      b.className = 'fm-editor-panel-container', this.createMlangPanel(b)
      for (var c = 0; c < a.length; c++) {
        var d = a[c]
        if (d.visible)var e = new AreaPanel(d, this); else var e = new ButtonPanel(d, this)
        b.appendChild(e.element), this.panels.push(e)
      }
      this.element.appendChild(b)
    }, createMlangPanel: function (a) {
      this.mlangPanel = new MlangPanel(this), a.appendChild(this.mlangPanel.element)
    }, redraw: function () {
      for (var a = 0; a < this.panels.length; a++) {
        var b = this.panels[a]
        b.redraw()
      }
    }, element: null, activePanel: null, mlangPanel: null, editor: null, panels: null, __class__: Toolbar
  }
}(), function () {
  'use strict'
  window.ArrayIterator = function (a) {
    ...
    }, cur: 0, arr: [], __class__: ArrayIterator
  }
}(), function () {
  'use strict'
  window.HashArray = function () {
    ...
    toString: function () {
      ...
      for (var b = this.keys(); b.hasNext();) {
        var c = b.next()
        a += c, a += ' => ', a += this.get(c), b.hasNext() && (a += ', ')
      }
      return a += '}'
    }, iterator: function () {
      return {
        ref: this.obj, it: this.keys(), hasNext: function () {
      ...
      var a = []
      for (var b in this.obj)this.obj.hasOwnProperty(b) && a.push(b.substr(1))
      return ArrayIterator(a)
    }, remove: function (a) {
      return a = '$' + a, this.obj.hasOwnProperty(a) ? (delete this.obj[a], !0) : !1
    }, exists: function (a) {
      return this.obj.hasOwnProperty('$' + a)
    }, get: function (a) {
      return this.obj['$' + a]
    }, set: function (a, b) {
      this.obj['$' + a] = b
    }, obj: null, __class__: HashArray
  }
}(), function () {
  'use strict'
  window.DomUtils = {
    hasClass: function (a, b) {
      ...
      if (a.classList)a.classList.remove(b); else if (this.hasClass(a, b)) {
      ...
      c && (d.onerror = function () {
        DomUtils.loadJsFile(c, b)
      }), d.onload = b, d.type = 'text/javascript', d.src = a, document.getElementsByTagName('head')[0].appendChild(d)
    }, replaceSVGUseWithGraphElements: function (a) {
      for (var b = a.getElementsByTagName('use'), c = [], d = [], e = 0; e < b.length; e++) {
        var f = b[e], g = f.getAttribute('href').replace('#', ''), h = document.getElementById(g).cloneNode(!0)
        h.id += '-c-' + (new Date).getTime()
        for (var i = {}, j = 0; j < f.attributes.length; j++) {
          var k = f.attributes[j]
          'href' !== k.nodeName && 'x' !== k.nodeName && 'y' !== k.nodeName ? h.setAttribute(k.nodeName, k.nodeValue) : ('x' == k.nodeName || 'y' == k.nodeName) && (i[k.nodeName] = k.nodeValue)
        }
        if (i.x) {
          var l = (i.x || 0) + ', ' + (i.y || 0), m = h.getAttribute('transform') || ''
          '' !== m && (m += ' '), m += 'translate(' + l + ')', h.setAttribute('transform', m)
        }
        c.push(h), d.push(f)
      }
      for (var e = 0; e < c.length; e++) {
        var n = d[e]
        d[e] = e, n.parentNode.replaceChild(c[e], n)
      }
    }
  }, HTMLTextAreaElement.prototype.insertAtCaret = function (a) {
    if (a = a || '', document.selection) {
      this.focus()
      var b = document.selection.createRange()
      b.text = a
    } else if (this.selectionStart || 0 === this.selectionStart) {
      var c = this.selectionStart, d = this.selectionEnd
      this.value = this.value.substring(0, c) + a + this.value.substring(d, this.value.length), this.selectionStart = c + a.length, this.selectionEnd = c + a.length
    } else this.value += a
    ...
    if (document.selection) {
      ...
      if (null == a)return 0
      var b = this.createTextRange(), c = b.duplicate()
      return b.moveToBookmark(a.getBookmark()), c.setEndPoint('EndToStart', b), c.text.length
    }
    return 0
  }, window.Url = {
    get get() {
      var a = {}
      return 0 !== window.location.search.length && window.location.search.replace(/[?&]+([^=&]+)=([^&]*)/gi, function (b, c, d) {
        c = decodeURIComponent(c), 'undefined' == typeof a[c] ? a[c] = decodeURIComponent(d) : a[c] = [].concat(a[c], decodeURIComponent(d))
      }), a
    }
  }
}()