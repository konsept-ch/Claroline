...
const SessionUsers = (props) => {
  const statusField = props.statusField || 'state'
  const statusLabel = props.statusLabel || trans('status')
  const statusChoices = props.statusChoices || constants.REGISTRATION_STATES
  const statusColors = props.statusColors || constants.REGISTRATION_STATE_COLORS
    ...
    <Fragment>
      <ListData
        ...
        definition={[
          ...
          render: (row) => (
            <span className={classes('label', `label-${statusColors[row[statusField]] || 'default'}`)}>
              {statusChoices[row[statusField]]}
            </span>
          )
        }
      ]}
...
SessionUsers.propTypes = {
  ...
  actions: T.func,
  statusField: T.string,
  statusLabel: T.string,
  statusChoices: T.object,
  statusColors: T.object,
  add: T.shape({
    // action types
  })
}
